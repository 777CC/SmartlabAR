...
//    	AR_MATRIX_CODE_GLOBAL_ID = 2830
    ...
    public string GetScreenShotPath(Rect croppedNumber,Vector2 pos0, Vector2 pos1, Vector2 pos2, Vector2 pos3)
    {
        if (_videoTexture0 != null) {
            Color[] colors = _videoTexture0.GetPixels();
            int width = _videoTexture0.width;
            

            Texture2D tex = FlipTexture( _videoTexture0);
            //Debug.Log("resolution : " + croppedNumber + " : " + tex.width + " : " + tex.height);


            //Rect croppedNumber = new Rect(0, 40, 480, 50);
            //Vector2 pos0 = new Vector2(0, 40);
            //Vector2 pos1 = new Vector2(0, 100);
            //Vector2 pos2 = new Vector2(200, 40);
            //Vector2 pos3 = new Vector2(200, 100);
            //int texW = 240;
            //int texH = 320;
            int texW = tex.width;
            int texH = tex.height;
            ...
            Vector2 cropedPixelPos = ScreenToPixelPos02(croppedNumber.x, croppedNumber.y, texW, texH);
            //Vector2 pixel0 = ScreenToPixelPos(pos0.x, pos0.y, texW, texH);
            //Vector2 pixel1 = ScreenToPixelPos(pos1.x, pos1.y, texW, texH);
            //Vector2 pixel2 = ScreenToPixelPos(pos2.x, pos2.y, texW, texH);
            //Vector2 pixel3 = ScreenToPixelPos(pos3.x, pos3.y, texW, texH);

            Vector2 pixel0 = ScreenToPixelPos01(pos0.x - croppedNumber.x, pos0.y - croppedNumber.y, texW, texH);
            Vector2 pixel1 = ScreenToPixelPos01(pos1.x - croppedNumber.x, pos1.y - croppedNumber.y, texW, texH);
            Vector2 pixel2 = ScreenToPixelPos01(pos2.x - croppedNumber.x, pos2.y - croppedNumber.y, texW, texH);
            Vector2 pixel3 = ScreenToPixelPos01(pos3.x - croppedNumber.x, pos3.y - croppedNumber.y, texW, texH);


            //int cropedTexX = (int)((croppedNumber.x * texW) / Screen.width);
            //int cropedTexY = (int)(((croppedNumber.y) * texH) / ((Screen.width * texH) / texW));
            //cropedTexY -= (((Screen.height * texW) / Screen.width) - texH) / 2;
            ...
            Debug.Log(pixel0 + " : " + pixel1 + " : " + pixel2 + " : " + pixel3);
            Texture2D croppedTex = new Texture2D(cropedTexWidth, cropedTexHeight);
            Debug.Log("croppedTex : " + croppedTex + " : " + cropedTexWidth + " : " + cropedTexHeight);
            //croppedTex.SetPixels(0, 0, cropedTexWidth, cropedTexHeight, tex.GetPixels(cropedTexX, cropedTexY, cropedTexWidth, cropedTexHeight));
            croppedTex.SetPixels(0, 0, cropedTexWidth, cropedTexHeight, tex.GetPixels((int)cropedPixelPos.x, (int)cropedPixelPos.y, cropedTexWidth, cropedTexHeight));
            Color maskCol = croppedTex.GetPixel((int)pixel0.x, (int)pixel0.y);
            ...
            File.WriteAllBytes(imagePath, croppedTex.EncodeToPNG());
            return "/ss.png";
        }
        else {
            return string.Empty;
    ...
    Vector2 ScreenToPixelPos01(float posX, float posY, int texWidth, int texHeight)
    {
    ...
    Vector2 ScreenToPixelPos02(float posX, float posY, int texWidth, int texHeight)
    {
        int screenWidth = Screen.width;
        int screenHeight = Screen.height;
    ...
    Texture2D FlipTexture(Texture2D original, bool upSideDown = true)
    {
        ...
        for (int i = 0; i < xN; i++)
        {
            for (int j = 0; j < yN; j++)
            {
                flipped.SetPixel(i, j, original.GetPixel(yN - j,xN - i));
                //if (upSideDown)
                //{
